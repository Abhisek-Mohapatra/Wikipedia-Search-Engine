...
def preprocess(data):
    return [stemmer.stemWord(token.casefold()) for token in tokenization(data) if token and token.casefold() not in stop_words]  # removal of stopwords done
...
def createSectionDict(text):
    ...
    for k in tokens:
        if len(k)<=2:
            ...
            temp[k] += 1  # key is word, value is count value
        else:
            temp[k]=1
...
def createPageDict(input_dict, index):
    global global_dict
    global page_dict  # {"word"=[title count,links count,category,info...etc]}
    for key in input_dict.keys():
        if key not in page_dict:
            page_dict[key]= [0,0,0,0,0,0]
        page_dict[key][index] += input_dict[key]   # for input _dict : word ->key, count->val

def createGlobalDict(doc_id):  
    for word in page_dict:
        lt = page_dict[word]  # list ['title count',...]
        tmpdict = {}  # key is doc id
        if word in global_dict:
            tmpdict = global_dict[word] # returns dictionary
        tmpdict[doc_id] = lt
        global_dict[word] = tmpdict   # global_dict :  {'word':{d1:[],d2:[]}}
...
#     print(filename)
    ...
    with open(inverted_index_file_path,"a") as of:
        for word,docs in sorted(global_dict.items()):
            ...
            docdict = docs
            docfreq = []
            for k,v in sorted(docdict.items()):  # k : doc_id, v: list of title count,info_count etc
                docdet = "d"+str(k)+"#"
                for i in range(6):
                    if v[i]>0:
    ...
    # d1#t4+i3+b2 (in docdict)  -> append it to docfreq list
    # final to write in file = word :d1#t4+i3+b2 | d2#t4+i3+b2   +\n  ==> in this format,content is added to inverted index file.

def preprocessTitle(title):
    title = cleanText(str(title))
    title_tokens = preprocess(title)   
    title_dict={}  # count of tokens  with token as key
    i=0
    for t in title_tokens:
        if len(t)<=2:
            continue
        elif t.isdecimal() and len(t)>4:  # removed nos greater than 4
            continue
...
def writeTitles():
    ...
    pickle.dump(titles, titles_file)  # dumping dictionary
    titleFileCount+=1
...
class WikiHandler(xml.sax.ContentHandler):
    ...
    def endElement(self,tag):
        ...
        if tag == "page":
            doc_id += 1 ## 1: (abc,12)  # 50000 docs
            titles[doc_id-1] = (self.title,self.tokens)
            page_dict.clear()
            if doc_id % documentLimit == 0:
                if global_dict:   # represents dictionary of dictionary
                    #print(global_dict)
                    ...
                    titles.clear()   
            self.tokens = 0
 #word :d1#t4+i3+b2 | d2#t4+i3+b2 
        if tag == "title":
            title_dict = preprocessTitle(self.title)
            createPageDict(title_dict, 0)   # records added to global page dictionary.... 0 index is assigned to title field
        ...
        if tag == "text":
            ...
            contentNoLinks = re.sub(externalLinksRegex,'',text)  # sub -> substitution with '' for pattern found in text
            ...
            body = re.sub(r'\{\{.*\}\}','', contentNoInfobox)  # text after removing links,category,info boxes by applying suitable regular expressions

            info_dict,body_dict,category_dict,references_dict,links_dict = dict(),dict(),dict(),dict(),dict()

            if (len(info)>0):
                info_dict = createSectionDict(info) # returns dictionary
            if (len(category)>0):
            ...
            self.tokens += (len(info_dict)+len(body_dict)+len(links_dict)+len(references_dict)+len(category_dict))  # update token count

            createPageDict(info_dict, 1)  # {"word",[title,info,body,category,links,references]}
...
def write_PrimaryIndices():
    ...
    for final_indexNum in range(1,11):
        ...
        pickle.dump(range_dict[final_indexNum], output_file)   # dumping a dictionary 
        output_file.close()
...
def kWayMerge():
    ...
    heap = [] 
    heapify(heap)    ### Implement Min Heap
    ...
    for file_num in range (1, indexFileCount+1):
        try:
            ...
            heappush(heap, (word, value, file_num))   # min heap
        except:
    ...
    while heap:
        ...
        while heap and heap[0][0] == word:   # we will check if next word in min heap is the same word or not...if it is , then we add that line to the current word
            line += "|"+ heap[0][1]
            try:
...
if __name__ == "__main__":
    ...
    if bool(titles):
        writeTitles()
        titles.clear()
    indexFileCount = math.ceil(doc_id/documentLimit)   # number of index files
    kWayMerge()
    writeToStats()
