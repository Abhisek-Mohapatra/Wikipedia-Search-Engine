...
def getIndexFile(word):
    if word[0] in check_dict['1']:   # checking 1st character
        return 1
...
def calPlainFrequency(posting_list):
    ...
    return numOfOccurences # calculates term frequency

def tfidfPlain(wordPostings):  # For PLain Query
    tfidfScores ={}
...
def calFieldFrequency(posting_list, current_field):   # (t4+b2+c3,t)
    Fields= ['t', 'b', 'i', 'c', 'r', 'l']
...
def tfidfField(wordPostings):
    tfidfScores ={}
    for word in wordPostings.keys():  # calculate for every word
        current_field= wordPostings[word][1]
        posting_lists= wordPostings[word][0].split('|')
        numOfDocs= len(posting_lists)  # posting list format : [t2+b3+r4,t1+b3+r34]
        totalDocs= TOTALDOCS
...
def getPostingList(fileWordMap):
    global index_dict   # File number as key -> content  for Index List
    wordPostings= {}
    for file_num, words in fileWordMap.items():
        for w in words:
            if w in index_dict[file_num]:
                wordPostings[w]= index_dict[file_num][w]   # key : word, value :"string like d1#t1+b3"  for Posting List
            else:
                wordPostings[w]= ''
    return wordPostings
    ...
    for file_num, entry in fileWordMap.items():
        for word, field in entry:
            if word in index_dict[file_num]:
                wordPostings[word]= (index_dict[file_num][word], field)    # key : word, value :"string like d1#t1+b3,title"  for Posting List
            else:
                wordPostings[word]= ('','')
    return wordPostings 
...
def preprocessFieldQuery(query):
    ...
    for f in fields:
        field= query.find(f)
        if field !=-1:
            fieldInfo[field]= f    # t,b,r   -> dc{0:t:,1:b:}
    
    fieldInfo= sorted(fieldInfo.items())  # returns a tuple  (index,field like t:,b:,r:)
    fieldInfo.append((1234, ""))            #fake dummy entry
    i=0
    while i+1 <len(fieldInfo):
        field= fieldInfo[i][1].strip(":")
        fieldQuery = (query[fieldInfo[i][0]+2 : fieldInfo[i+1][0]]).lower()  #  query :t:sachin b:Tendular
...
def getTop(tfidfScores):
    ...
    K = 10
    kRelevant = []
    for key, value in tfidfScores.items():   # key is documentId
        if(K ==0):
            break
        x=math.ceil(int(key)/documentLimit)
        kRelevant.append((key, titles_dict[x][int(key)][0]))   # (doc Id,title) ---> Tuple
        K -=1
    return kRelevant
...
if __name__ == "__main__" :
    ...
    for query in query_list:
        FileWordMap = dict()
        if ":" in query:     # Field Query
            FileWordMap= preprocessFieldQuery(query)    #  t:Sachin b:Tendulkar c:Sports
            # print(FileWordMap)
            ...
            FileWordMap = preprocessPlainQuery(query)   # Sachin Tendulkar
            loadInvertedIndex(FileWordMap)
            ts = time.time()
            postingLists = getPostingList(FileWordMap)  # posting list is for given query
            tfidfScores = tfidfPlain(postingLists)
            te = time.time()

        time1 = te-ts
        tsf = time.time()
        tfidfScores= OrderedDict(sorted(tfidfScores.items(), key=lambda t: t[1], reverse= True))   #(docid,score)
